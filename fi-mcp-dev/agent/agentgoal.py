...
from promptgoal import GOAL_OPTIMIZATION_PROMPT
from analyzer import analyze_transactions
...
finance_agent = Agent(
    model=MODEL_NAME,
    name="FinanceAgent",
    instruction=GOAL_OPTIMIZATION_PROMPT,
    tools=[]
)
...
async def async_main(): # Keep this as async because asyncio.run() needs an awaitable.
    ...
    try:
        raw_data = fetch_transactions() + fetch_stock() + fetch_credit() + fetch_epf() + fetch_mf() + fetch_networth()
        ...
        user_input = input("Give goal name and value: ")
        full_prompt_for_llm = f"{GOAL_OPTIMIZATION_PROMPT}\n\nHere is the financial data:\n{raw_data} \n\n The User's goal is {user_input}"
